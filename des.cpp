...
char* DES::bitsetToCharArray(std::bitset<64> bits) {
    std::string bitsetString = bits.to_string();
    char* charArray = (char*)bitsetString.c_str();
    return charArray;
}

bitset<64> DES::show_encryp() {
...
//    QString myGreet = QString::fromLocal8Bit(bitsetToCharArray(cipher));
//    qDebug()<<;
    return cipher;
}

void DES::show_decrypt(bitset<64> temp) {
     // читаем файл a.txt
     // Расшифровываем и записываем в файл b.txt

    bitset<64> temp_plain = DES_decrypt(temp);
    bitset<64> temp_1 = change(temp_plain);
    QString myGreet = QString::fromUtf8(bitsetToCharArray(temp_1));
    qDebug()<<"output"<<myGreet;
